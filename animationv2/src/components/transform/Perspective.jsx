...
export default function Perspective() {
    ...
    linksRef.current = dataperspective.map((element, i) => linksRef.current[i] ?? React.createRef());
        ...
        >
            <div className="push">
                <div className="main">
                    ...
                    {
                        dataperspective.map((data, key) => <Section
                            sections={sections}
                            counter={key + 27}
                            code={data.code}
                            copy={data.copy}
                ...
                <div className="aside">
                    <Menu links={links}>
                        {
                            dataperspective.map((link, key) => <li key={key} ref={linksRef.current[key]}><a id={`l${key}`} href={`#s${key}`}>{link.header}</a></li>)
                        }
                    </Menu>
                </div>
            </div>
