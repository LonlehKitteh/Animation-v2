...
export default function TransformStyle() {
    ...
    linksRef.current = datatransformstyle.map((element, i) => linksRef.current[i] ?? React.createRef());
        ...
        >
            <div className="push">
                <div className="main">
                    ...
                    {
                        datatransformstyle.map((data, key) => <Section
                            sections={sections}
                            counter={key + 23}
                            code={data.code}
                            copy={data.copy}
                ...
                <div className="aside">
                    <Menu links={links}>
                        {
                            datatransformstyle.map((link, key) => <li key={key} ref={linksRef.current[key]}><a id={`l${key}`} href={`#s${key}`}>{link.header}</a></li>)
                        }
                    </Menu>
                </div>
            </div>
